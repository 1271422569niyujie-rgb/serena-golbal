...
- 投资雷达明确区分 Nasdaq 100（NDX）和苹果股市常见的 Nasdaq Composite（IXIC）；黄金同时显示 COMEX 美元/盎司和工行 Au99.99 元/克参考价。Au99.99 是 1000g 标准金锭交割规格参考，不等于品牌金条零售价。
...
- “外面的人已经走到哪里了”只回答未来 1–3 年的结构变化，用两句大白话说明变化和意义；依据链接降到次要位置，不再写成新闻摘要。
- 认知更新允许只有 1–3 条，前台每条只显示一句话；`data/cognition-history.json` 保存最近 30 天的核心结论和去重键，生成前先做语义去重。
...
行情真实性保护：Yahoo 返回的空值不会再被转换成 `0`；0 或负数不能通过发布校验。工行参考价来自其公开贵金属行情页，抓取失败时只让该项显示“暂不可用”，不会拿其他黄金价格冒充。
